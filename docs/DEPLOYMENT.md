...
| `FRED_API_KEY` | ❌ | `src/app/api/feeds/fred/series/route.ts` | Free Federal Reserve Economic Data API key (instant registration at https://fred.stlouisfed.org/docs/api/api_key.html). Drives 25 macro/financial nodes (Fed Funds, CPI, Unemployment, JOLTS, mortgage rates, Case-Shiller, Labor Force Participation, GDP QoQ, PPI, Forward Inflation, Wheat Price, etc.). When unset, the route returns deterministic mock data per series tagged `(mock — FRED_API_KEY unset)`. |
| `WORLD_BANK_BASE_URL` | ❌ | `src/app/api/feeds/world-bank/series/route.ts` | Override for the upstream World Bank Open Data API (default: `https://api.worldbank.org/v2`). Optional: no key needed because WB publishes anonymously. Drives 5 country-level macro nodes (Brazil/China GDP + Employment, Global CPI Inflation YoY). |
| `OPENFDA_API_KEY` | ❌ | `src/app/api/feeds/openfda/events/route.ts` | Optional FDA OpenFDA API key for higher rate limits (free, registers at https://open.fda.gov/apis/authentication/). Drives T1D drug nodes (Teplizumab, Insulin Glargine) with adverse-event report counts from FAERS. Without a key the public anonymous limit is sufficient for the 2 queries on a 6h cadence. |
