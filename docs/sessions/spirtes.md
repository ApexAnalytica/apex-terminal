...
- ~~Hardening of latent-confounder detection (FCI) for production graphs.~~ ✅ shipped (FCI v0.3) — `src/lib/discovery/algorithms/fci.ts`. Skeleton phase (PC-stable) + v-structure orientation + Zhang's R1 + R2 + R3 (the discriminating "kite") orientation rules as a fixed-point pass. Returns a Partial Ancestral Graph with `endpointMarks: { sourceMark, targetMark }` (`circle`/`arrow`/`tail`) on every edge. The orientation phase is refactored into directly-testable helpers — `buildInitialEdges`, `applyVStructures`, `applyR1`, `applyR2`, `applyR3` — each returning whether a mark changed, with `runOrientation` as the fixed-point composer. 20 unit tests covering pattern recovery, output-shape contracts, plus dedicated fixture tests for R1, R2 (triangle directed-sub-path triggers + negatives), and R3 (kite + negatives). Open follow-ups: orientation rule R4 (long discriminating paths); nonparametric CI tests. UI wiring has an architectural prerequisite — the FCI panel consumes `CausalGraph` (curated runtime graph) while FCI consumes `Cohort` (subjects × measurements). Wiring FCI's PAG output into the panel needs either a CausalGraph→Cohort bridge or a stored `DiscoveryRun` to load; honest path is via Phase-2 Spirtes-live.
