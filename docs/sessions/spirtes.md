...
- ~~Hardening of latent-confounder detection (FCI) for production graphs.~~ ✅ shipped (FCI v0.2) — `src/lib/discovery/algorithms/fci.ts`. Skeleton phase (PC-stable) + v-structure orientation + Zhang's R1 + R2 orientation rules (fixed-point pass), returns a Partial Ancestral Graph with `endpointMarks: { sourceMark, targetMark }` (`circle`/`arrow`/`tail`) on every edge. Linear-Gaussian CI tests via the existing `partialCorrelation` helper. Registered in `algorithm-registry.ts`. 12 unit tests cover independence, chain, fork, collider patterns, output-shape contracts, plus two R1 fixture tests (constructed `SkeletonResult` — exported for testing). Open follow-ups: orientation rules R3 / R4 (discriminating paths); dedicated R2 fixture test; nonparametric CI tests. UI wiring has an architectural prerequisite — the FCI panel consumes `CausalGraph` (curated runtime graph) while FCI consumes `Cohort` (subjects × measurements). Wiring FCI's PAG output into the panel needs either a CausalGraph→Cohort bridge or a stored `DiscoveryRun` to load; honest path is via Phase-2 Spirtes-live.
