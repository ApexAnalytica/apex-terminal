...
- ~~DCD / NOTEARS — third leg of the SPIRTES algorithm trio.~~ ✅ shipped (NOTEARS v0.1) — `src/lib/discovery/algorithms/notears.ts`. Continuous DAG-structure learning via the smooth acyclicity constraint h(W) = tr(e^(W∘W)) − d. Linear-Gaussian model, augmented-Lagrangian outer loop, proximal gradient descent inner loop with L1 sparsity. Standardises columns to mitigate varsortability (Reisach et al. 2021). New `_matrix-ops.ts` helper module (matmul, matExp via 20-term Taylor series, Hadamard, trace, transpose). Registered in `algorithm-registry.ts`. 18 unit tests: 6 matrix-op sanity, 5 pattern recovery (independence, chain adjacency, fork adjacency, acyclicity, no self-loops), 4 output-shape contracts, and 3 varsortability stress tests (Reisach et al. 2021) — adjacency stability under column permutation (Jaccard ≥ 0.5), acyclicity preserved across representative permutations (identity / swap / reverse), edge-magnitude bounded under the reverse-permutation varsortability adversary. Open follow-ups: NOTEARS-MLP (nonlinear via neural nets); L-BFGS inner loop instead of plain proximal gradient (faster convergence on larger d).
- ~~Hardening of latent-confounder detection (FCI) for production graphs.~~ ✅ shipped (FCI v0.4) — `src/lib/discovery/algorithms/fci.ts`. Skeleton phase (PC-stable) + v-structure orientation + Zhang's R1 + R2 + R3 (kite) + R4 (length-3 discriminating path) orientation rules as a fixed-point pass. Returns a Partial Ancestral Graph with `endpointMarks: { sourceMark, targetMark }` (`circle`/`arrow`/`tail`) on every edge. The orientation phase is refactored into directly-testable helpers — `buildInitialEdges`, `applyVStructures`, `applyR1`, `applyR2`, `applyR3`, `applyR4` — each returning whether a mark changed, with `runOrientation` as the fixed-point composer. 24 unit tests covering pattern recovery, output-shape contracts, plus dedicated fixture tests for R1, R2 (triggers + negatives), R3 (kite + negatives), and R4 (B-in-sepset → directed; B-not-in-sepset → bidirected; precondition negatives). Open follow-ups: longer (>3-edge) discriminating paths in R4 (requires path search); nonparametric CI tests. UI wiring has an architectural prerequisite — the FCI panel consumes `CausalGraph` (curated runtime graph) while FCI consumes `Cohort` (subjects × measurements). Wiring FCI's PAG output into the panel needs either a CausalGraph→Cohort bridge or a stored `DiscoveryRun` to load; honest path is via Phase-2 Spirtes-live.
