...
- Three stacked blocks at the top of `ModulePanel` (the right panel) that give the user immediate situational awareness:
  ...
  - **CALCULATIONS** (`CalculationsPanel.tsx`) — pure-function readouts from `src/lib/calculations/` registry (HHI, cross-domain edges, mean ΩF). New entries (Greeks, T1D scores, supply-chain variants) plug in by appending to `CALCULATION_REGISTRY`. Calculations that implement the optional `toSnapshot(result, ctx)` method gain a "→ DIAL" button that pushes the current value into the selected node's `liveData[]` via `pushCalculationSnapshot`, where it accumulates history through the existing temporal infrastructure (renders as a sparkline in the time-series cards, scrubable on the TimeDial). HHI is the first calc opted in
