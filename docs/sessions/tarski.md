...
> **Status:** Active. Live API feeds wired into A-04 (Hormuz throughput), R-01 / R-02 (sanctions), and R-04 (cross-domain confidence × WGI governance). Live Coverage Program: 7 providers shipped (EIA, OFAC, FRED, World Bank, OpenFDA, ClinicalTrials.gov, Derivations) covering **~56 graph nodes** including the T1D side, EM FX, sovereign default, MENA import dependency, the broader CPI / PCE / wage / sentiment expansion, and WGI Rule of Law for the China / Brazil jurisdictions. **Real-data-only goal reached: 0 synthetic composites remaining** — all 4 originally synthetic composites are now live-derived from real data. All free-tier; mock fallback when keys/upstream missing.
>
