...
- **PCMCI+ (v0.1)** — extends `pcmci-linear` with a contemporaneous skeleton + orientation phase. Reuses the lagged PCMCI for `X[t-k] → Y[t]` edges (no duplication), then tests partial correlation `X[t] ↔ Y[t]` conditional on the union of lagged parents of both endpoints (BH-FDR over candidates only). Orients via **lagged-parent imbalance** — if X has a lagged parent Y doesn't, the edge is oriented X → Y; otherwise emitted as undirected with `endpointMarks: {sourceMark: "circle", targetMark: "circle"}` per FCI convention. Honest about the v0.1 limitations (no contemporaneous PC-stable with separating-set tracking, no v-structure detection from separating sets, no Meek propagation, linear/Gaussian only) — in realistic data with contemporaneous coupling, most edges emit as undirected because the lagged phase finds spurious lagged paths through the contemporaneous chain.
