...
- ~~DCD / NOTEARS — third leg of the SPIRTES algorithm trio.~~ ✅ shipped (NOTEARS v0.1) — `src/lib/discovery/algorithms/notears.ts`. Continuous DAG-structure learning via the smooth acyclicity constraint h(W) = tr(e^(W∘W)) − d. Linear-Gaussian model, augmented-Lagrangian outer loop, proximal gradient descent inner loop with L1 sparsity. Standardises columns to mitigate varsortability (Reisach et al. 2021). New `_matrix-ops.ts` helper module (matmul, matExp via 20-term Taylor series, Hadamard, trace, transpose). Registered in `algorithm-registry.ts`. 15 unit tests covering: matrix-op sanity (matExp(0)=I, matExp on nilpotent gives tr=d, matExp on cyclic gives tr>d), pattern recovery (independence, chain adjacency, fork adjacency — direction recovery is approximate under symmetric Gaussian noise per the literature), acyclicity convergence (|h(W)| < 5e-3), no self-loops, and output-shape contracts. Open follow-ups: NOTEARS-MLP (nonlinear via neural nets); L-BFGS inner loop instead of plain proximal gradient (faster convergence on larger d); Reisach varsortability stress tests.
