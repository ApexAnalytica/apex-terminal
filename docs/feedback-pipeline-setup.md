...
| `GITHUB_WEBHOOK_SECRET` | Random 32+ char string | PR-merge webhook HMAC verification |
| `VERCEL_WEBHOOK_SECRET` | From Vercel webhook setup (step 5) | Deploy-succeeded webhook verification |
...
## 4. GitHub webhook (PR merge → in_progress)
...
- **Events**: "Let me select individual events" → tick **Pull requests** only
...
## 5. Vercel deploy webhook (deploy-succeeded → shipped)

Vercel team dashboard → Settings → Webhooks → Create Webhook:

- **Project(s)**: apex-terminal
- **Events**: `deployment.succeeded` (or `deployment-ready` depending on Vercel UI)
- **URL**: `https://<your-domain>/api/webhooks/vercel`

Copy the generated secret into `VERCEL_WEBHOOK_SECRET`.

The handler flips every `in_progress` row to `shipped` on any successful prod deploy. That's "best-effort" on purpose: single-main-branch + auto-deploy means any merged in_progress PR ends up in the next prod deploy, so batching is fine.
...
6. Vercel builds + deploys → Vercel webhook fires → row flips to `shipped` with `shipped_at`.
...
- **Submitter notification**: not wired in v1. Drop a Resend call into the Vercel webhook after the update if you want the PM to get an email on ship.
